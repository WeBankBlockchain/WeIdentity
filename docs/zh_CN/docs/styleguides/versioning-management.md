...
1. SDK 需要升级，智能合约不升级。如果 SDK 已经有版本修复了这个 bug，并且 SDK 的版本号只有 `MINOR` 和 `PATCH` 版本号跟当前使用的 SDK 不一致，因为这类 SDK 的升级保证了向后兼容性，从而肯定能兼容当前使用的智能合约，所以可以直接升级 SDK。例如使用 `weid-java-sdk-v1.2.3` 的过程中遇到了 bug，然后这个 bug 在 `weid-java-sdk-v1.3.0` 版本中得到了修复，则可以直接升级。
    > 注意:
