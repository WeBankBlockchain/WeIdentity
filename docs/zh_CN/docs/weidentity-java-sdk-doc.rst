...
* `WeIdentity JAVA SDK 安装部署文档 <https://weidentity.readthedocs.io/zh_CN/latest/docs/one-stop-experience.html>`_
...
.. code-block:: text
   ...
   接口定义:ResponseData<Boolean> addLogByHash(String hashValueSupplement（仅在customKey中用到）, String hashValue / customKey, String log, WeIdPrivateKey weIdPrivateKey)
   接口描述: 为一个**已经在链上存在的存证**添加签名，同时添加额外信息记录，存入其log中。有两个接口，一个是以hash值为索引，一个可以接受用户自定义索引（customKey）；如果自定义索引不存在，则会使用替补hash作为上链索引。
...
.. code-block:: text
   ...
   接口描述: 为一个**已经在链上存在的存证**的某个Signer发起，撤销、或反撤销此存证。只会记录最后的一次撤销/反撤销操作状态。
...
.. mermaid::
   ...
   participant EvidenceService
   participant 区块链节点
   调用者->>EvidenceService: 调用revoke()
   EvidenceService->>EvidenceService: 入参非空、格式及合法性检查
   opt 入参校验失败
   EvidenceService-->>调用者: 报错，提示参数不合法并退出
   end
   EvidenceService->>区块链节点: 调用智能合约，写入撤销事件
   区块链节点-->>EvidenceService: 返回查询结果
   opt 查询出错
   EvidenceService-->>调用者: 报错并退出
   end
...
.. code-block:: text
   ...
   接口定义:ResponseData<Boolean> revoke(EvidenceInfo evidenceInfo, String weId)
   接口描述: 检查某个Evidence的某个Signer是否已经撤销了此签名。需要首先使用getEvidence获取链上存证。
...
.. mermaid::
   ...
   participant EvidenceService
   participant 区块链节点
   调用者->>EvidenceService: 调用isRevoked()
   EvidenceService->>EvidenceService: 入参非空、格式及合法性检查
   opt 入参校验失败
   EvidenceService-->>调用者: 报错，提示参数不合法并退出
   end
   opt 查询未撤销
   EvidenceService-->>调用者: 报失败
   end
   EvidenceService-->>调用者: 返回成功
